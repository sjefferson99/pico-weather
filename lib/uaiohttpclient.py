...
async def request_raw(method, url, headers=None, data: str = ""):
    try:
...
async def request(method, url, headers=None, data: str = ""):
    redir_cnt = 0
    while redir_cnt < 2:
        reader = await request_raw(method, url, headers, data)
        headers = []
